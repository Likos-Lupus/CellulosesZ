...
public final class ExperienceMath {
    ...
    public static int pointsToNextLevel(int level) {
        ...
        if (level >= 30) return Math.addExact(Math.multiplyExact(9, level), -158);
        if (level >= 15) return Math.addExact(Math.multiplyExact(5, level), -38);
    ...
    public static int levelForTotal(int total) {
        ...
        while (low < high) {
            ...
            if (totalForLevel(middle) <= total) low = middle;
            else high = middle - 1;
        }
        return low;
    }
    ...
    public static int totalForLevel(int level) {
        ...
        if (result > Integer.MAX_VALUE) throw new IllegalArgumentException("level is too large");
        return (int) result;
    }
