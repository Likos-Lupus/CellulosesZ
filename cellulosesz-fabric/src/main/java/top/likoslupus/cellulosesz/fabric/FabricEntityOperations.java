...
public final class FabricEntityOperations implements EntityPlatformService {
    ...
    public FabricEntityOperations(FabricPlatformService platform) {
    ...
    public boolean validLivingEntity(String entityId) {
        ...
        if (type == null || DENIED_TYPES.contains(type) || !type.canSummon()) return false;
        var level = platform.requireServer().overworld();
    ...
    public PlatformResult<SpawnMobResult> spawnMob(SpawnMobRequest request) {
        ...
        return onServerThread(() -> {
            ...
            var type = BuiltInRegistries.ENTITY_TYPE.getOptional(location).orElse(null);
            if (type == null || DENIED_TYPES.contains(type) || !type.canSummon()) {
                return PlatformResult.failure(PlatformOperationStatus.INVALID_ARGUMENT, "Entity type is not allowed");
            }
            var anchor = platform.nativePlayer(request.anchor());
            ...
            if (!level.hasChunkAt(base) || !level.getWorldBorder().isWithinBounds(base)) {
                return PlatformResult.failure(PlatformOperationStatus.STATE_NOT_ALLOWED, "Spawn position is unavailable");
            }
            var spawned = 0;
            for (var index = 0; index < request.amount(); index++) {
                ...
                entity.moveTo(base.getX() + 0.5D, base.getY(), base.getZ() + 0.5D, anchor.getYRot(), 0.0F);
            ...
            var result = new SpawnMobResult(location.toString(), request.amount(), spawned);
            if (spawned == request.amount()) return PlatformResult.success(result);
            if (spawned > 0) return PlatformResult.partial(result, "Some entities could not be spawned");
            return PlatformResult.failure(PlatformOperationStatus.STATE_NOT_ALLOWED, "No entities were spawned");
        });
    }
    ...
    public PlatformResult<ProjectileLaunchResult> launchProjectile(ProjectileRequest request) {
        requireNonNull(request, "request");
        return onServerThread(() -> {
            var shooter = platform.nativePlayer(request.shooter());
            var level = shooter.level();
            var entity = createProjectile(level, request.type());
            if (!(entity instanceof Projectile projectile)) {
                if (entity != null) entity.discard();
                return PlatformResult.failure(PlatformOperationStatus.UNSUPPORTED, "Projectile type is unavailable");
            }
            var direction = shooter.getLookAngle();
            if (!validDirection(direction)) {
                entity.discard();
                return PlatformResult.failure(PlatformOperationStatus.STATE_NOT_ALLOWED, "View direction is invalid");
            }
            entity.setPos(shooter.getX(), shooter.getEyeY() - 0.1D, shooter.getZ());
            projectile.setOwner(shooter);
            projectile.shoot(direction.x, direction.y, direction.z, (float) request.speed(), 0.0F);
            if (!level.addFreshEntity(projectile)) {
                projectile.discard();
                return PlatformResult.failure(PlatformOperationStatus.STATE_NOT_ALLOWED, "Projectile could not be spawned");
            }
            tracked.put(projectile.getUUID(), new TrackedEntity(
            ...
            return PlatformResult.success(new ProjectileLaunchResult(projectile.getUUID(), request.type()));
        });
    }
    ...
    public PlatformResult<TntBurstResult> spawnTnt(TntBurstRequest request) {
        requireNonNull(request, "request");
        return onServerThread(() -> {
            var level = platform.serverLevel(request.center().world);
            if (level.isEmpty())
            ...
            var spawned = 0;
            for (var index = 0; index < request.amount(); index++) {
                var angle = (Math.PI * 2.0D * index) / Math.max(1, request.amount());
                var radius = request.amount() == 1 ? 0.0D : request.spread() * ((index % 5) / 4.0D);
                ...
                if (!targetLevel.hasChunkAt(position) || !targetLevel.getWorldBorder().isWithinBounds(position))
                    continue;
                var tnt = new PrimedTnt(targetLevel, x, y, z, null);
                tnt.setFuse(request.fuseTicks());
                if (!targetLevel.addFreshEntity(tnt)) continue;
                tracked.put(tnt.getUUID(), new TrackedEntity(
            ...
            if (spawned == request.amount()) return PlatformResult.success(result);
            if (spawned > 0) return PlatformResult.partial(result, "Some TNT entities could not be spawned");
            return PlatformResult.failure(PlatformOperationStatus.STATE_NOT_ALLOWED, "No TNT entities were spawned");
        });
    }
    ...
    public PlatformResult<TemporaryMobResult> launchTemporaryMob(TemporaryMobRequest request) {
        ...
        return onServerThread(() -> {
            var shooter = platform.nativePlayer(request.shooter());
            var level = shooter.level();
            Entity entity = switch (request.type()) {
            ...
            if (!(entity instanceof LivingEntity living)) {
                if (entity != null) entity.discard();
                return PlatformResult.failure(PlatformOperationStatus.INTERNAL_ERROR, "Temporary entity creation failed");
            }
            ...
            if (!level.addFreshEntity(living)) {
                living.discard();
                return PlatformResult.failure(PlatformOperationStatus.STATE_NOT_ALLOWED, "Temporary entity could not be spawned");
            }
            tracked.put(living.getUUID(), new TrackedEntity(
                    level.dimension().identifier().toString(),
                    ticks + request.lifetimeTicks(),
                    request.explosionPower(),
                    request.blockDamage(),
    ...
    @Override
    public void tick() {
        if (!platform.requireServer().isSameThread()) return;
        ticks++;
        var removals = new ArrayList<UUID>();
        tracked.forEach((uuid, state) -> {
            var level = platform.serverLevel(state.worldId()).orElse(null);
            if (level == null) {
                removals.add(uuid);
                return;
            }
            ...
            var detonate = state.customTnt() && entity instanceof PrimedTnt tnt && tnt.getFuse() <= 1;
            var impact = !state.customTnt() && (entity.onGround() || entity.horizontalCollision || entity.verticalCollision);
            if (ticks >= state.expiresAt() || detonate || impact) {
    ...
    public void clearTrackedEntities() {
        if (!platform.requireServer().isSameThread()) return;
        tracked.forEach((uuid, state) -> platform.serverLevel(state.worldId())
                .map(level -> level.getEntity(uuid))
                .ifPresent(Entity::discard));
        tracked.clear();
    }

    private void explode(ServerLevel level, Vec3 position, TrackedEntity state) {
        if (state.explosionPower() <= 0.0D) return;
        level.explode(
                null,
                position.x,
                position.y,
                ...
                state.blockDamage() ? Level.ExplosionInteraction.TNT : Level.ExplosionInteraction.NONE
        );
    }
    ...
    private static boolean validDirection(Vec3 direction) {
        return Double.isFinite(direction.x) && Double.isFinite(direction.y) && Double.isFinite(direction.z)
