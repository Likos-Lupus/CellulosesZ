...
final class SignTransactionTest {

    @Test
    void trade_whenAtomicCommitFails_doesNotReportSuccess() {
        ...
        var items = proxy(
                ...
                (method, args) -> switch (method.getName()) {
                    case "parse" -> PlatformResult.success(new ItemDescriptor(
        ...
        var message = assertInstanceOf(LocalizedMessage.class, result.message());
        assertEquals("service.sign.trade-inventory-changed", message.key());
    }
    ...
    @Test
    void sell_whenDepositAndRollbackFail_reportsRollbackSeparately() {
        var mutation = new InventoryMutation() {
            @Override
            public PlatformResult<Void> commit() {
        ...
        assertFalse(result.success());
        var message = assertInstanceOf(LocalizedMessage.class, result.message());
        assertEquals("service.sign.sell-rollback-failed", message.key());
    }
